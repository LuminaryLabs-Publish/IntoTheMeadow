...
export function createGrassClumpArchetypeKit(config = {}) {
  const families = Object.freeze(config.families ?? ["short", "tall", "golden", "shadow", "flower-edge"]);
  const atlas = config.atlas ?? "meadow-grass-atlas";

  function createArchetype(family = "short", variant = 0) {
    const cardCount = Math.max(50, Math.min(100, Number(config.cardCount ?? (family === "tall" ? 88 : 64))));
    const radius = family === "tall" ? 2.4 : family === "flower-edge" ? 2.0 : 1.75;
    const heightMin = family === "short" ? 0.22 : family === "tall" ? 0.55 : 0.32;
    const heightMax = family === "short" ? 0.9 : family === "tall" ? 1.7 : 1.18;
    const localCards = Object.freeze(Array.from({ length: cardCount }, (_, i) => {
      ...
      return Object.freeze({
        ...
        width: 0.08 + hashUnit(`${family}:${variant}:w:${i}`) * 0.12,
        height: h,
        uvRect: Object.freeze({ x: (i % 4) / 4, y: Math.floor(i % 16 / 4) / 4, w: 0.25, h: 0.25 }),
        bendWeight: 0.35 + hashUnit(`${family}:${variant}:bend:${i}`) * 0.65
      });
    }));
    return Object.freeze({
